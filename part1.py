...
def show_of_hands(set1: "List", set2: "List") -> "returns which hands wins":
    ...
    if number_of_cards == 3:
        ...
        # Check for 'Pair'
        if (player1_vals[0] == player1_vals[1] or
           player1_vals[1] == player1_vals[2] or
           player1_vals[2] == player1_vals[0]):
            return set1, "Pair, Player 1 Wins!"
        elif(player2_vals[0] == player2_vals[1] or
           player2_vals[1] == player2_vals[2] or
           player2_vals[2] == player2_vals[0]):
    ...
    # 5 Card Poker game
        ...
        if(player2_vals_counter.most_common()[0][1] == 2 and player2_vals_counter.most_common()[1][1] == 2):
            player2_win_flag = True
        if (player1_win_flag and player2_win_flag):
            if (max(player1_vals_counter.most_common()[0][0], player1_vals_counter.most_common()[1][0])) \
                > (max(player2_vals_counter.most_common()[0][0], player2_vals_counter.most_common()[1][0])):
